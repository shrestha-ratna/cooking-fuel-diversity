...
knit: (function(input_file, encoding) {
 ...
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, '04_01_permanova_test.html'))})
---
...
#### Permanova Test Using Euclidean Distance: Model with interaction
```{r}
# GET DISTANCE MATRIX ---
...
# PERMANOVA TEST ---
set.seed(12345)
y_permanova <- vegan::adonis(dist_dml ~ IncomeGroup*Ethnicity + Ethnicity*EducationLevel+ Ethnicity*GeoRegion + IncomeGroup*EducationLevel+EducationLevel*GeoRegion+IncomeGroup*GeoRegion, data=dml, permutations=999, method="euclidean", parallel=4)
y_permanova

# Write Output ---
file.output <- file.path(dir.output, "stats_abundance_permanova_1.tsv")
write.table(as.data.frame(y_permanova$aov.tab), file.output, sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
```

#### Permanova Test Using Euclidean Distance: Model without interaction
```{r}
# PERMANOVA TEST ---
set.seed(12345)
y_permanova <- vegan::adonis(dist_dml ~ IncomeGroup+Ethnicity+EducationLevel+GeoRegion, data=dml, permutations=999, method="euclidean", parallel=4)
y_permanova

# Write Output ---
file.output <- file.path(dir.output, "stats_abundance_permanova_2.tsv")
write.table(as.data.frame(y_permanova$aov.tab), file.output, sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
```
...
# PERMANOVA TEST ---
set.seed(12345)
y_permanova <- vegan::adonis(dist_dml ~ IncomeGroup*Ethnicity + Ethnicity*EducationLevel+ Ethnicity*GeoRegion + IncomeGroup*EducationLevel+EducationLevel*GeoRegion+IncomeGroup*GeoRegion, data=dml, permutations=999, method="bray", parallel=4)
y_permanova
